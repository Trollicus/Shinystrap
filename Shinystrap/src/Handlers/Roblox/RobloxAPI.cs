...
public class RobloxApi
{
    ...
    {
        var request = await _handler.SendAsync("https://auth.roblox.com/v1/authentication-ticket", HttpMethod.Post,
            new[]
            {
    ...
    {
        var request = await _handler.SendAsync("https://auth.roblox.com/v1/authentication-ticket", HttpMethod.Post,
            $"{{ \"clientAssertion\": \"{await ClientAssertion(cookie)}\" }}", new[]
            {
                new HttpHandler.RequestHeadersEx("Referer", "https://www.roblox.com/"),
                new HttpHandler.RequestHeadersEx("User-Agent",
                    "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:150.0) Gecko/20100101 Firefox/150.0"),
    ...
    {
        using var key =
            Registry.CurrentUser.OpenSubKey(@"Software\ROBLOX Corporation\Environments\RobloxPlayer\Channel");
    ...
    private async Task<Dictionary<string, List<string>>> GetPlayerTokens(string gameId, string cookie)
    {
        ...
        do
        {
            ...
            nextPageCursor = root.TryGetProperty("nextPageCursor", out var next) &&
    ...
    public async Task JoinServerThroughId(string cookie, string placeId, string serverId)
    {
        ...
        Process.Start(new ProcessStartInfo
        {
            FileName = robloxExe,
            Arguments =
                $"--app -t {authTicket} -j https://www.roblox.com/Game/PlaceLauncher.ashx?request=RequestGame&browserTrackerId={DateNow()}&placeId={placeId}&isPlayTogetherGame=false&referredByPlayerId=0&joinAttemptId={serverId}&joinAttemptOrigin=PlayButton -LaunchExp InApp"
        });
    }
    ...
    private class Search
    {
        ...
        {
            [JsonPropertyName("universeId")] public object universeId { get; set; }

            [JsonPropertyName("name")] public string name { get; set; }
            
            [JsonPropertyName("description")] public string description { get; set; }
            
            [JsonPropertyName("rootPlaceId")] public object rootPlaceId { get; set; }
        }

    }
    
    public async Task<string> GetGameThumbnail(object universeId)
    {
    ...
    public async Task<List<GameList.GameCard>> SearchGameAsync(string text)
    {
        var request =
            await _handler.SendAsync($"https://apis.roblox.com/search-api/omni-search?searchQuery={text}&sessionId=a",
                HttpMethod.Get);
        
        var response = await request.Content.ReadAsStringAsync();
        
        var data = JsonSerializer.Deserialize<Search>(response);
        
        if (data?.searchResults == null)
            return new List<GameList.GameCard>();
        
        var games = data.searchResults
        ...
        {
            var image = await GetGameThumbnail(game.universeId);

            return new GameList.GameCard
            {
                Title = game.name,
                Description = "Roblox experience",//game.description, make fix for max length and fix somehow the images not showing
                ImageUrl = image,
